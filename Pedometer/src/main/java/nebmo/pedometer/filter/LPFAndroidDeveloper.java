package nebmo.pedometer.filter;
...
public class LPFAndroidDeveloper implements LowPassFilter
{
	...
	public float[] addSamples(float[] acceleration)
	{
		...
		if (count > 5)
		{
			...
			linearAcceleration[3] = (float)Math.sqrt(Math.pow(linearAcceleration[0],2)+Math.pow(linearAcceleration[1],2)+Math.pow(linearAcceleration[2],2));
		}
